...
namespace Integrador.Iugu.Lib
{
    ...
    public class Account : IDisposable
    {
        ...
        /// <param name="accountId">Id da conta a ser validada</param>
        /// <returns>Resposta da API de validação da conta</returns>
        public async Task<SimpleResponseMessage> UpdateAccountAsync(AccountUpdateRequestMessage accountData, string accountId, string userToken)
        {
            var retorno = await Api.PutAsync<SimpleResponseMessage>(accountData, $"{accountId}", userToken).ConfigureAwait(false);
            return retorno;
        }
